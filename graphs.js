    ...
    d3.csv("household types.csv", function makeGraphs(error, HouseholdJson) {
        d3.csv("industry types.csv", function makeGraphs(error, IndustryJson) {
            d3.csv("veteran.csv", function makeGraphs(error, VeteranJson) {
                d3.csv("crime.csv", function makeGraphs(error, CrimeJson) {
                ...
                populationProjects.forEach(function (d) {
                    ...
                    for (var k = 0; k < CrimeJson.length; k++) {
                        if (CrimeJson[k].FIPS == d.Name && CrimeJson[k].YEAR == d.year) {
                            for (var j = 2; j < col2.length; j++) {
                                d[col2[j]] = +CrimeJson[k][col2[j]]
                            }
                        }
                    }
                ...
                var numViolentByDate = dateDim.group().reduceSum(function (d) {
                    return d["CPOPARST"];
                });

                var numPropertyByDate = dateDim.group().reduceSum(function (d) {
                    return d["PROPERTY"];
                });
                ...
                //var crimeChart = dc.compositeChart("#crime-chart");
                var crimeBarChart = dc.barChart("#crime-bar-chart");
                //var crimetest = dc.lineChart('#crime-line-chart');
                ...
                veteranChart
                    ...
                    .renderHorizontalGridLines(true)
                    .legend(dc.legend().x(450).y(50).itemHeight(13).gap(5))
                    .compose([dc.lineChart(crimeChart).dimension(dateDim).group(numViolentByDate, "Violent Crime").colors('blue').dashStyle([2, 2]),
                        dc.lineChart(crimeChart).dimension(dateDim).group(numPropertyByDate, "Property Crime").colors('red').dashStyle([2, 2])
                    ])
                    .brushOn(false)
                    .render();
                ...
                crimeBarChart
                    ...
                    .dimension(dateDim)
                    .group(numViolentByDate, "Violent Crime")
                    .stack(numPropertyByDate, "Property Crime")
                    .transitionDuration(500)
                    //.x(d3.scale.linear().domain([2009, 2015]))
                    .x(d3.scale.ordinal().domain([2010, 2011, 2012, 2013, 2014]))
                    .xUnits(dc.units.ordinal)
